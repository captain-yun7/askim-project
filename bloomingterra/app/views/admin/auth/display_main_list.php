<script>
	function delete_display_theme(){
		if($(":checkbox[name='no[]']:checked").length < 1) {
			alert("선택된 서비스 사례 진열이 없습니다.");
			return false;
		}

		if(!confirm("선택된 서비스 사례 진열을 삭제하시겠습니까?")) {
			return false;
		}
...
<div id="contents">
	<div class="main_tit">
		...
		<div class="btn_right btn_num2">
			...
			<a href="display_main_reg" class="btn point new_plus">+ 서비스 사례 진열 등록</a>
		</div><!--btn_right-->
	...
	<div class="terms_privecy_box">
		<dl>
			<dt>- 메인 진열 서비스 사례를 프론트페이지(홈페이지)에서 보이게하려면?</dt>
			<dd>
			메인 진열 서비스 사례를 생성하고나면, <em class="point">"치환코드"</em> 항목이 보이실겁니다. <br>
			해당 치환코드를 보여주고싶은 <em class="point">홈페이지 스킨명/index.html</em> 안에 <em>html 소스작업</em>을 하여 넣어주셔야 합니다.<br>
			예 ) {=include_display_main('display_main', '1')} 치환태그를 메인 진열 서비스 사례가 나왔으면 하는 곳에 넣어주세요.<br><br>
			</dd>
		</dl>
		<dl>
			<dt>- 메인 진열 서비스 사례가 나오는 곳 디자인을 바꾸고 싶어요.</dt>
			<dd>
			메인 진열 서비스 사례 치환태그는, <em class="point">홈페이지 스킨명/layout/display/메인 진열 서비스 사례 설정에서 선택한 스킨리스트.html</em>을 메인으로 불러오게끔 하는 역할을 합니다.<br/>
			즉, 메인 진열 서비스 사례에 적용되는 html 태그는 모두 <em>layout/display</em>안에 html 파일 형태로 있는 거지요.<br>
			새로운 디자인을 추가하려면, 해당 폴더에 새로운 html 파일을 추가하고, 메인 진열 서비스 사례 설정하는 곳에서 스킨리스트를 변경해주시면 됩니다.<br><br>
			</dd>
		</dl>
	</div>
</div>