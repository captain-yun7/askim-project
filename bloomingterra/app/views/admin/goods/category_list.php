					...
					if(request.readyState == "4" && request.status == "200") {
						...
						if(result.code) {
							if(result.data.cnt > 0) {
								alert("해당 카테고리 안에 서비스 사례가 있습니다.\n서비스 사례가 있으면 삭제가 되지 않습니다.");
								return false;
							}
		...
		<div class="cate_select_box">
			...
			<div class="terms_privecy_box">
				...
				<dl>
					...
					<dd>
					카테고리 사용유무 기능은, 홈페이지에서 카테고리 및 카테고리에 속한 서비스 사례 전체를 미노출 시키는 기능입니다.<br>
					아직 홈페이지에서 노출시키고 싶지 않은 카테고리의 경우, "사용유무"를 "사용안함"으로 선택하세요.<br/>
					* 관리자페이지에서 서비스 사례 등록 및 하위 카테고리 생성, 카테고리 설정값 수정은 가능합니다.<br><br>
					</dd>
				</dl>
				<dl>
					<dt>- 카테고리 "활성유무"는 어떤 기능인가요?</dt>
					<dd>
					카테고리 활성유무 기능은, 홈페이지에서 해당 카테고리에 속한 서비스 사례의 상세페이지 진입을 제한하는 기능입니다.<br>
