...
if($browser['name']=="Google Chrome"){
	$name = $_COOKIE['kakao_username'];
}else{
	$name = iconv("EUC-KR","UTF-8",$_COOKIE['kakao_username']);
}

if(!empty($_COOKIE['kakao_id'])){

	$_SESSION['wiz_session']['id']          = "KK".$_COOKIE['kakao_id'];
	$_SESSION['wiz_session']['passwd']      = $_COOKIE['kakao_id'];
	$_SESSION['wiz_session']['name']        = $name;
	$_SESSION['wiz_session']['email']       = $_COOKIE['kakao_email'];
	...
	if($deny_id == true) {
		...
		if($_COOKIE['prev_page'] == "" && mobile_check()) $_COOKIE['prev_page'] = "/";
		...
		echo "<script> 
			...
			pdate.setSeconds(pdate.getSeconds()-1);
			document.cookie = 'prev_page=\"\"; expires='+pdate.toGMTString()+'; path=/';
			opener.document.location='$_COOKIE['prev_page']'; 
			self.close();
			</script>";
		}
