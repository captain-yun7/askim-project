...
if($browser['name']=="Google Chrome"){
	$name = $_COOKIE['google_username'];
}else{
	$name = $_COOKIE['google_username'];
}


if(!empty($_COOKIE['google_id'])){

	$_SESSION['wiz_session']['id']          = "GG".$_COOKIE['google_id'];
	$_SESSION['wiz_session']['passwd']      = $_COOKIE['google_id'];
	$_SESSION['wiz_session']['name']        = $name;
	$_SESSION['wiz_session']['email']       = $_COOKIE['google_email'];
	...
	if($deny_id == true) {
		...
		echo "<script> 
			...
			pdate.setSeconds(pdate.getSeconds()-1);
			document.cookie = 'prev_page=\"\"; expires='+pdate.toGMTString()+'; path=/';
			location.href='$_COOKIE['prev_page']'; 
			self.close();
			</script>";
		}
