...
if($browser['name']=="Google Chrome"){
	$name = $_COOKIE['naver_username'];
}else{
	$name = iconv("EUC-KR","UTF-8",$_COOKIE['naver_username']);
}


if(!empty($_COOKIE['naver_id'])){

	$_SESSION['wiz_session']['id']          = $_COOKIE['naver_id'];
	$_SESSION['wiz_session']['sns_id']      = $_COOKIE['naver_snsId'];
	$_SESSION['wiz_session']['passwd']      = $_COOKIE['naver_id'];
	$_SESSION['wiz_session']['name']        = $name;
	$_SESSION['wiz_session']['email']       = $_COOKIE['naver_email'];
	...
	if($deny_id == true) {
		...
		if($_COOKIE['PM']=="M" || $_COOKIE['prev_page'] == "/m"){
			...
			echo "<script> 
				...
				pdate.setSeconds(pdate.getSeconds()-1);
				document.cookie = 'prev_page=\"\"; expires='+pdate.toGMTString()+'; path=/';
				opener.location='$_COOKIE['prev_page']'; 
				self.close();
				</script>";
		}
