...
cheditor.prototype = {
    ...
    resetEditArea : function () {
        ...
        this.addEvent(self.doc, 'paste', function (event) {
...
//        this.editArea.focus();
    ...
    makeHtmlContent : function () {
        return this.cheditor.textContent.value;
    },
        ...
        this.getNodeTree(this.doc.body, function (node) {
            ...
            switch (node.type) {
                ...
                case GB.node.element :
                    ...
                    if (GB.newLineBeforeTags[node.name]) {
    ...
    doOnKeyUp : function (evt) {
        var keyCode = evt.keyCode, br, node, range, storedRange, enterNode, storedNode;
        ...
        enterNode = range.commonAncestorContainer;
        ...
        if (enterNode.lastChild && enterNode.lastChild.nodeName.toLowerCase() === 'br') {
            enterNode.lastChild.className = this.cheditor.bogusSpacerName;
            return;
        }
    ...
    popupWinLoad : function (popupAttr) {
        var self = this,
            ...
            popWinResizeHeight = function (evt) {
                ...
                self.cheditor.popupElem.style.top = Math.round(parseInt(self.cheditor.popupElem.style.top, 10) +
                        Math.round((self.cheditor.editArea.clientHeight - self.cheditor.popupElem.clientHeight) / 2)) + 'px';
    ...
    getRange : function () {
        var sel = this.getSelection(), range = null;
        if (this.W3CRange) {
            if (sel.getRangeAt) {
                range = (sel.rangeCount > 0) ? sel.getRangeAt(0) : this.doc.createRange();
            } else {
            ...
            range = sel.createRange ? sel.createRange() : this.doc.createRange();
            if (!range) {
