...
echo "<script type=\"text/javascript\" src=\"http://checkout.naver.com/customer/js/checkoutButton2.js\" charset=\"UTF-8\"></script>".PHP_EOL;
}
		...
		if(addok == "Y"){
			...
			if(document.prdForm.optcode4 != null){
				...
				if(document.prdForm.optcode4.value == ""){
					...
					document.prdForm.opt_price4.value = "";
					document.prdForm.opt_reserve4.value = "";
					setSellprice();
					...
					document.prdForm.opt_price4.value = optlist[1];
					document.prdForm.opt_reserve4.value = optlist[2];
					setSellprice();
					add_option(4,optval);
...
function checkOpt05(){
	...
	var optval = document.prdForm.optcode5.value;
	var optlist = optval.split("^");
	var addok = "Y";
	var basket_cnt = $("#basket_cnt").val();
...
function checkOpt06(){
	...
	var optval = document.prdForm.optcode6.value;
	var optlist = optval.split("^");
	var addok = "Y";
	var basket_cnt = $("#basket_cnt").val();
...
function checkOpt07(){
	...
	var optval = document.prdForm.optcode7.value;
	var optlist = optval.split("^");
	var addok = "Y";
	var basket_cnt = $("#basket_cnt").val();
...
function trremove(no,opt,num,opname){
...
function add_option(opt,optval){
	...
	if(opt == 2){
		...
		html += "	<td width='45%' style='padding:0 0 0 10px'><strong>"+optlist[0]+"</strong></td>\n";
		html += "	<td align=center width='86'>\n";
		html += "	<div>\n";
		html += "		<img src='/twcenter/images/but_vol_down.gif' onclick='javascript:decAmount2("+no+","+opt+");' style='cursor:pointer'>\n";
		html += "		<input ntype='text' name='basket_amount"+no+"' id='basket_amount"+no+"' value='1' size='2' class='amout_num2' onChange='checkAmount2("+no+","+opt+")' onkeyup='inputAmount("+no+")'>\n";
		...
		html += "			<input class='btn_type4' onclick='trremove("+no+","+opt+",2,\""+optlist[0]+"\")' type='button' value='삭제'>\n";
		html += "		</div>\n";
		html += "	</td>\n";
		html += "</tr>\n";
...
//		var t_price    = parseInt(optlist[1]);
		...
		tmp_sellprice = (opttitle != "") ? 0 : tmp_sellprice;
...
function total_prd_price(){
	...
	for(var i=1;i<=basket_cnt+1;i++){
		...
		if(tmp_sellprice && opt_price){	
			...
			<?php } else if($opttitle == "") { ?>
				<?php if(($opttitle5 != "" || $opttitle6 != "" || $opttitle7 != "") && ($opttitle3 != "" || $opttitle4 != "" || $opttitle8 != "" || $opttitle9 != "" || $opttitle10 != "" || $opttitle11 != "")) {
				?>
	...
	if(total_price == 0){
		...
		?>
		total_price = parseInt("<?php echo $prd_row['sellprice'] ?>");
		<?php
		}
		?>
...
function ajax_insert(type){
	...
	if(basket_cnt == "0" || bb_cnt == "") {
		if(opt_use == 'Y') {
			alert('(필수) <?=$opttitle?>옵션을 선택해주세요.');
		} else {
			alert("옵션을 선택해주시기 바랍니다.");
		}
	} else {
		var bottom_form_val = "";
	...
	?>
		...
		if(bb_cnt == "0" || bb_cnt == "") {
			...
			for(var i=1;i<=basket_cnt;i++){
				...
				var ajaxParam = $("#bottom_form"+i).serialize()+"&direct=basket";
				$.ajax({
					type:"post"
					, async: false
...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
	<!-- <tr>
		<td height="35" align="right" class="here">HOME<strong><?=$cate_lo?></strong></td>
	</tr>
	...
	<tr>
		...
		<!-- 실제 컨텐츠 부분 -->
			<table width="100%" border="0" cellpadding="0" cellspacing="0">
							...
							<tr>
								...
								<?php } else { ?>
									<table border=0 cellpadding=0 cellspacing=0>
										<tr>
											<td align=center style="border:1px solid #cdcdcd; width:450px; height:450px;" class="prdimg"><img src="<?php echo $prdimg ?>" name="prdimg"></td>
										</tr>
										<tr>
											<td align=center style="padding:10px 0px;">
												<table border=0 cellpadding=0 cellspacing=0>
													<tr>
														<td><button type="button" onClick="location.href='<?=$prev_prdcode?>'" class="btn_type5">Prev</button></td>
														<td><button type="button" onClick="prdZoom();" class="btn_type5">+ &nbsp; Zoom</button></td>
														<td><button type="button" onClick="location.href='<?=$next_prdcode?>'" class="btn_type5">Next</button></td>
													</tr>
												</table>
											</td>
										</tr>
								...
								<input type="hidden" name="basket_cnt"    id="basket_cnt"    value="0">
									...
									<table width="100%" border=0 cellpadding=0 cellspacing=0>
										<tr>
											<td class="p_name" style="padding-bottom:13px;"><?php echo $prdname ?></td>
										</tr>
										<tr>
											<td class="price_con">
												<table border="0" cellpadding="0" cellspacing="0" width="100%">
													<tr>
														<td width="20%" class="p_tit">판매가</td>
														<td class="p_con">
													...
													<tr>
														...
														<td class="p_con"><?php echo number_format($member_price) ?><?php echo $lev_row->distype ?> [<?php echo $level ?>]</td>
													</tr>
													<?php
														}
													}
													...
													<tr>
														...
														<td id="coupon" class="p_con"><?php echo number_format($coupon_price) ?>원 &nbsp;<?php echo number_format($prd_row['coupon_dis']) ?><?php echo $coupon_type ?>&nbsp;<a href="/twcenter/product/coupon_down.php?prdcode=<?php echo $prdcode ?>"><img src="/twcenter/images/coupon_down.gif" border="0"></a></td>
													</tr>
													<?php
														}
													...
													<tr>
														...
														<td class="p_con"><span id="reserve_default"><?php echo number_format($reserve) ?></span>원</td>
													</tr>
													<?php
														}
													}
										...
										<tr>
											<td style="padding:15px 0 10px 20px;">
												...
												<table border="0" cellpadding="0" cellspacing="0" width="100%">
													...
													<?php if($sp_img != ""){ ?>
													<tr>
														<td class="p_tit">상품상태</td>
														<td class="p_con"><?php echo $sp_img ?></td>
													</tr>
													<?php } ?>
													<?php if($prdcom != ""){ ?>
													<tr>
														<td class="p_tit">제조사</td>
														<td class="p_con"><?php echo $prdcom ?></td>
													</tr>
													<?php } ?>
													<?php if($origin != ""){ ?>
													<tr>
														<td class="p_tit">원산지</td>
														<td class="p_con"><?php echo $origin ?></td>
													</tr>
													<?php } ?>
													...
													?>
													<tr>
														<td class="p_tit"><?php echo ${"info_name".$ii} ?></td>
														<td class="p_con"><?php echo ${"info_value".$ii} ?></td>
													</tr>
													<?php
															}
														}
													...
													?>
													<tr>
														<td class="p_tit">수량</td>
														<td class="p_con">
															<table border=0 cellpadding=0 cellspacing=0>
																<tr>
																    <td><a href="javascript:decAmount();"><img src="/twcenter/images/but_vol_down.gif" border=0></a></td>
																	<td><input type=text name="amount" id="amount" value="1" size="2" class="amout_num2" onkeyup="default_amount();"></td>
													...
													?>
													<tr>
														<td class="p_tit"><?php echo $opttitle ?><?php echo $opttitle2 ?></td>
														<td class="p_con">
														<?php
													...
													<?php if($opttitle5 != ""){ ?>
													<tr>
														<td class="p_tit"><?php echo $opttitle5 ?></td>
														<td class="p_con">
															<select name="optcode5"  id="id_optcode5" onChange="checkOpt05()">
													...
													<?php if($opttitle6 != ""){ ?>
													<tr>
														<td class="p_tit"><?php echo $opttitle6 ?></td>
														<td class="p_con">
															<select name="optcode6" id="id_optcode6" onChange="checkOpt06()">
																<option value=""> 선택하세요 </option>
																<?php
													...
													<?php if($opttitle7 != ""){ ?>
													<tr>
														<td class="p_tit"><?php echo $opttitle7 ?></td>
														<td class="p_con">
															<select name="optcode7" id="id_optcode7" onChange="checkOpt07()">
																<option value=""> 선택하세요 </option>
																<?php
													...
													<?php if($opttitle3 != ""){ ?>
													<tr>
														<td class="p_tit"><?php echo $opttitle3 ?></td>
														<td class="p_con">
															<select name="optcode3" id="id_optcode3" onChange="checkOpt03()">
																<option value=""> 선택하세요 </option>
																<?php
													...
													<?php if($opttitle4 != ""){ ?>
													<tr>
														<td class="p_tit"><?php echo $opttitle4 ?></td>
														<td class="p_con">
															<select name="optcode4" id="id_optcode4" onChange="checkOpt04()">
																<option value=""> 선택하세요 </option>
																<?php
													...
													<?php if($opttitle8 != ""){ ?>
													<tr>
														<td class="p_tit"><?php echo $opttitle8 ?></td>
														<td class="p_con">
															<select name="optcode8" id="id_optcode8" onChange="checkOpt08()">
																<option value=""> 선택하세요 </option>
																<?php
													...
													<?php if($opttitle9 != ""){ ?>
													<tr>
														<td class="p_tit"><?php echo $opttitle9 ?></td>
														<td class="p_con">
															<select name="optcode9" id="id_optcode9" onChange="checkOpt09()">
																<option value=""> 선택하세요 </option>
																<?php
													...
													<?php if($opttitle10 != ""){ ?>
													<tr>
														<td class="p_tit"><?php echo $opttitle10 ?></td>
														<td class="p_con">
															<select name="optcode10" id="id_optcode10" onChange="checkOpt10()">
																<option value=""> 선택하세요 </option>
																<?php
													...
													<?php if($opttitle11 != ""){ ?>
													<tr>
														<td class="p_tit"><?php echo $opttitle11 ?></td>
														<td class="p_con">
															<select name="optcode11" id="id_optcode11" onChange="checkOpt11()">
																<option value=""> 선택하세요 </option>
																<?php
													...
													<tr>
														...
														<td class="p_con"> <img src="/twcenter/images/icon_star_<?php echo $prefer ?>.gif"></td>
													</tr>
													<?// }
													*/ ?>
												</table>
										...
										<tr>
											<td style="padding:9px 0 5px 20px;">
												<table border="0" cellpadding="0" cellspacing="0" width="100%">
													<tr>
														<td class="p_tit" width="20%">SNS 스크랩</td>
														<td class="p_con">
										...
										<tr>
											<td style="padding:15px 0;">
												...
												?>
												<table width="100%" cellpadding="0" cellspacing="0" border="1" class="opt_table">
													<tr>
										...
										<tr>
											<td style="padding:7px 0;">
												...
												?>
													<tr>
														<td align="right">
															<span style="font-size:15px; font-weight:600; color:#EB0009">총 상품 금액&nbsp;&nbsp;&nbsp;&nbsp;</span>
										...
										<td style="padding:20px 0 0 0">
												<table width="100%" border="0" cellpadding="0" cellspacing="0">
													...
													if(empty($strprice)) {
														...
														if($opttitle == "" && $opttitle2 == "" && $opttitle3 == "" && $opttitle4 == "" && $opttitle5 == "" && $opttitle6 == "" && $opttitle7 == "" && $opttitle8 == "" && $opttitle9 == "" && $opttitle10 == "" && $opttitle11 == ""){
													?>
															<input class="buy_btn" onclick="javascript:saveBasket('buy');" type="button" value="바로구매">
															<input class="basket_btn" onclick="javascript:saveBasket('basket');" type="button" value="장바구니">
															<input class="wish_btn" onclick="javascript:saveWish();" type="button" value="관심상품">
													<?php } else { ?>
													        <input class="buy_btn" onclick="javascript:ajax_insert('buy');" type="button" value="바로구매">
															<input class="basket_btn" onclick="javascript:ajax_insert('basket');" type="button" value="장바구니">
															<input class="wish_btn" onclick="javascript:ajax_insert_wish('wish');" type="button" value="관심상품">
													<?php
														}
													}
													?>
															<input class="wish_btn" onclick="location.href='<?php echo $_SERVER['PHP_SELF'] ?>?catcode=<?php echo $catcode ?>&brand=<?php echo $brand ?>&page=<?php echo $page ?>&<?php echo $param ?>'" type="button" value="목록으로">
														</td>
													</tr>
												</table>
											</td>
										...
										<tr>
											...
											<script type="text/javascript" >//<![CDATA[
												nhn.CheckoutButton.apply({
													...
													WISHLIST_BUTTON_LINK_URL:"http://<?php echo $_SERVER['HTTP_HOST'] ?>/twcenter/product/nhn_wish.php?prdcode=<?php echo $prdcode ?>", //⑨
													"":""
												});
											//]]></script>
														</td>
					...
					<!-- 상세 정보-->
					 <div class="prd_tab">
					  <ul>
						...
						<?php if(!strcmp($oper_info['qna_usetype'], "Y")) { ?><li><a href="#qna">상품 Q&amp;A</a></li><? } ?>
						<?php if(!strcmp($oper_info['review_usetype'], "Y")) { ?><li><a href="#review">상품후기</a></li><? } ?>
					  </ul>
					</div>
						<table width="100%" border="0" cellpadding="0" cellspacing="0">
							<?php
							...
							<?php } ?>
							<tr>
								<td colspan="5" style="padding:0 5px;"><?php echo $content ?></td>
							</tr>
						</table>
					</td>
				</tr>
				...
				<tr><td colspan="5" align=center valign=top><?php echo $page_info->content ?></td></tr>

				<tr><td colspan="5" height="50"></td></tr>

				<tr>
					<td colspan="5" ><div id="rel">
					<!-- 관련상품 -->
