	...
	<tr>
		<td>
			<table width='100%' border='0' cellpadding='5' cellspacing='0'>
				<tr>
					<td width='60%' height='35' align='center' bgcolor='#f9f9f9'><strong>상품정보</strong></td>
...
if($oper_info['reserve_use'] == "Y"){
	$ordmail .= "<td width='10%' align='center' bgcolor='#f9f9f9'><strong>적립금</strong></td>";
}
$ordmail .= "
				</tr>
				<tr>
	...
	while($row = sql_fetch_obj($result)){
		...
		else $row->prdimg = "http://".$_SERVER['HTTP_HOST']."/twcenter/data/prdimg/".$row->prdimg;
		...
		if(strpos($row->optcode5,"&&") !== false){
			...
			if(!isset($t_optcode5_v2)) $t_optcode5_v2 = 0.00;
		...
		if(strpos($row->optcode6,"&&") !== false){
			...
			if(!isset($t_optcode6_v2)) $t_optcode6_v2 = 0.00;
		...
		if(strpos($row->optcode7,"&&") !== false){
			...
			if(!isset($t_optcode7_v2)) $t_optcode7_v2 = 0.00;
...
";
if($oper_info['reserve_use'] == "Y"){
	$ordmail .= "<td align='center'>".number_format($row->prdreserve*$row->amount)."원</td>";
}
$ordmail .= "
				</tr>
				<tr>
					...
					<td height='50' colspan='5' bgcolor='#f9f9f9'>
						<table width='100%' border='0' cellpadding='0' cellspacing='0'>
							<tr>
								...
								<td align='right' style='padding-right:10px;'>상품(<strong>".number_format($order_info['prd_price'])."원</strong>) ".$discount_msg." + 배송비(<strong>".$deliver_price_view." 원</strong>) ".$coupon_msg.$reserve_msg." = 주문합계 <span style='color:#d43d60; font-weight:bold;'>".number_format($order_info['total_price'])."원</span></td>
							</tr>
						</table>
					</td>
				</tr>
...
$ordmail .= "
				<tr>
				   <td bgcolor='#f9f9f9' style='padding-left:10px; border-right:1px solid #d7d7d7; border-bottom:1px solid #d7d7d7;'><strong>입금계좌</strong></td>
				   <td style='border-bottom:1px solid #d7d7d7; padding:5px; ' colspan='3'>".$order_info['account']." <font color=red>(가상계좌번호로 입금하셔야 주문이 완료됩니다.)</font></td>
				</tr>";
