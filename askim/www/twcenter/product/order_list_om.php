...
$period_sql = " AND order_date between '$prev_period' and '$next_period' ";
...
<div class="order_list_top">
	<div class="left">
		...
		<div class="cal">
			<span><label for="prev_date" class="blind">날짜</label><input type="text" name="prev_date" id="prev_date" class="input2" value="<?php echo $week_day2 ?>"></span><span class="hipen">~</span>
			<span><label for="next_date" class="blind">날짜</label><input type="text" name="next_date" id="next_date" class="input2" value="<?php echo $to_day2 ?>"></span>
		</div>
	</div>
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="order_table">
	<tr>
		<td width="14%" class="table_tit">주문일자</td>
		<td width="15%" class="table_tit">주문번호</td>
		<td width="22%" class="table_tit">결제금액</td>
		<!-- <td width="10%" class="table_tit">결제방법</td> -->
		<td class="table_tit">배송상태</td>
		<!-- <td class="table_tit">운송장번호</td> -->
		<td width="13%" class="table_tit">상세보기</td>
		<td width="13%" class="table_tit">영수증</td>
	</tr>
<?php
if($wiz_session['id'] != ""){
...
while(($row = sql_fetch_obj($result)) && $rows){
	...
	$ord_view_page = $_SERVER['PHP_SELF']."?ptype=view&orderid=".$row->orderid."&page=".$page."&".$param;
...
?>
	<tr>
	  ...
	  <td align="center"><a href="<?php echo $ord_view_page ?>"><?php echo $row->orderid ?></a></td>
	  <td align="center"><?php echo number_format($row->total_price) ?>원<span class="prd_table_stxt">( <?php echo pay_method($row->pay_method) ?> )</span></td>
	  <td align="center"><font color="<?php echo $stacolor ?>"><?php echo order_status($row->status) ?></font><?php if($row->deliver_num == '' ){ ?><?}else{?><span class="prd_table_stxt">(<?php echo $del_com ?> :  <?php echo $row->deliver_num ?>)</span><p class="tpad_5">
	  <a href="javascript:;" onclick="delivery_status_popup('<?php echo $del_trace ?>','<?php echo $row->deliver_num ?>')" title="<?php echo $del_com ?>" class="btn_type1">배송조회</a></p>
	  <?}?></td>
	  <td align="center"><!-- <a href="<?php echo $ord_view_page ?>"><img src="/twcenter/product/image/btn_detail.gif" border="0" align="absmiddle" /></a> -->
		  <input type="button" onClick="location.href='<?php echo $ord_view_page ?>'" value="보기" class="btn_type1">
	  </td>
	  <td align="center">
		<?php if(in_array($row->pay_method, $receipt_pay_method_array)) { ?>
		<?php echo receipt_link($oper_info, $row) ?>
	    <?php } ?>
	  </td>
	</tr>
<?php
...
if($total <= 0){
	echo "<tr><td colspan=15 align=center height=50 >주문내역이 없습니다.</td></tr>";
}
?>
</table>
...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
  <tr>
    <td align="center" height="80"><?php print_pagelist($page, $lists, $page_count, $param); ?></td>
  </tr>
</table>