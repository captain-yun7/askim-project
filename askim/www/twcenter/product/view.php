...
if($prd_row["color_use"] == "Y"){
	include_once $_SERVER["DOCUMENT_ROOT"]."/twcenter/product/view2.php";
	return;
}
...
$shortexp    = nl2br($prd_row['stortexp']);
...
$brand_idx		= $prd_row['brand']; 

//등록된 브랜드가 있으면 해당 브랜드 가져오기 2021-03-24 정나혜
if($brand_idx > 0){
	$brand_sql = "select idx, brdname from wiz_brand where idx='$brand_idx'";
	$brand_res = query($brand_sql);
	$brand_row = sql_fetch_obj($brand_res);

	$brand_name = $brand_row->brdname;
}


// 상품아이콘
if($prd_row['popular'] == "Y") 	$sp_img .= "<img src='/twcenter/images/icon_hit.gif' alt='hit'/>&nbsp;";
if($prd_row['recom'] == "Y") 	$sp_img .= "<img src='/twcenter/images/icon_rec.gif' alt='recom'/>&nbsp;";
if($prd_row['newc'] == "Y") 	$sp_img .= "<img src='/twcenter/images/icon_new.gif' alt='new'/>&nbsp;";
if($prd_row['best'] == "Y") 	$sp_img .= "<img src='/twcenter/images/icon_best.gif' alt='best'/>&nbsp;";
if($prd_row['sale'] == "Y")		$sp_img .= "<img src='/twcenter/images/icon_sale.gif' alt='sale'/>&nbsp;";
...
function setSellprice(){
	...
	if($opttitle  == "" && $opttitle2 == "" && $opttitle3 == "" && $opttitle4 == "" && $opttitle5 == "" && 
	   $opttitle6 == "" && $opttitle7 == "" && $opttitle8 == "" && $opttitle9 == "" && $opttitle10 == "" && $opttitle11 == "") {
	 ?>
...
function add_option(name){
	...
	if(newcheckOption() && newcheckOpt01()){
		...
		if(duplicate_chk == "N" || $("#basket_cnt").val() == "0"){
			...
			html += "<div>";
			html += "<img src='/twcenter/images/but_vol_down.gif' onclick='javascript:downAmount("+no+");' style='cursor:pointer' alt='down'/>";
			html += "<label for='amount_sel_"+no+"' class='blind'>수량</label><input name='amount_sel_"+no+"' id='amount_sel_"+no+"'type='text' size='2' value='1' class='amout_num2' onChange='inputAmount("+no+")' onkeyup='inputAmount("+no+")'>";
			html += "<img src='/twcenter/images/but_vol_up.gif' onclick='javascript:upAmount("+no+");' style='cursor:pointer' alt='up'/>";
			html += "</div>";
			html += "</td>";
...
<div class="pro_view">
		...
		<div class="simpleLens-container">
			<div class="simpleLens-big-image-container" style="border:1px solid #cdcdcd; width:500px; height:500px;">
				<a class="simpleLens-lens-image" data-lens-image="<?php echo $prdimg_L ?>">
					<img src="<?php echo $prdimg ?>" class="simpleLens-big-image" alt="이미지"/>
				</a>
			</div>
		</div>
		<div class="imgBtn">
	...
	<div class="price_con">
		...
		<div class="viewTable">
		<table class="view_table" summary="상품정보">
		...
		<tr>
			...
			<td>
				<select name="optcode5"  id="optcode5" onChange="OptionAdd('optcode5','5')" title="옵션값">
		...
		<tr>
			...
			<td>
				<select name="optcode6" id="optcode6" onChange="OptionAdd('optcode6','6')" title="옵션값">
					<option value=""> 선택하세요 </option>
					<?
		...
		<tr>
			...
			<td>
				<select name="optcode7"  id="optcode7" onChange="OptionAdd('optcode7','7')" title="옵션값">
					<option value=""> 선택하세요 </option>
					<?
		...
		<tr>
			...
			<td>
				<select name="optcode3" id="optcode3" onChange="OptionAdd('optcode3','3')" title="옵션값">
					<option value=""> 선택하세요 </option>
					<?
		...
		<tr>
			...
			<td>
				<select name="optcode4" id="optcode4" onChange="OptionAdd('optcode4','4')" title="옵션값">
					<option value=""> 선택하세요 </option>
					<?
		...
		<tr>
			...
			<td>
				<select name="optcode8" id="optcode8" onChange="OptionAdd('optcode8','8')" title="옵션값">
					<option value=""> 선택하세요 </option>
					<?
		...
		<tr>
			...
			<td>
				<select name="optcode9" id="optcode9" onChange="OptionAdd('optcode9','9')" title="옵션값">
					<option value=""> 선택하세요 </option>
					<?
		...
		<tr>
			...
			<td>
				<select name="optcode10" id="optcode10" onChange="OptionAdd('optcode10','10')" title="옵션값">
					<option value=""> 선택하세요 </option>
					<?
		...
		<tr>
			...
			<td>
				<select name="optcode11" id="optcode11" onChange="OptionAdd('optcode11','11')" title="옵션값">
					<option value=""> 선택하세요 </option>
					<?
...
<div id="info" class="bpad_50">
 <div class="prd_tab">
  <ul>
	...
	<?php if(!strcmp($oper_info['qna_usetype'], "Y")) {  ?><li><a href="#qna">상품 Q&amp;A <span class="review_num">(<?=$qna_cnt?>)</span></a></li><?php }  ?>
