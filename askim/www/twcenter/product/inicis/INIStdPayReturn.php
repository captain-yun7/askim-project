...
if(!strcmp($oper_info['pay_test'], "Y")) {
	$signKey = $oper_info['pay_key'];
}else{
	$signKey = $oper_info['pay_key'];
}

// 주문정보 조회
$sql = "SELECT * FROM wiz_order WHERE orderid='".$_REQUEST["orderNumber"]."'";
$result = query($sql);
            ...
            if (strcmp("0000", $_REQUEST["resultCode"]) == 0) {
                ...
                try {
					...
					if($order_info->pay_method == "PV"){
						...
						$_Payment['bankkind']		= $resultMap['VACT_BankCode'];	//은행코드
						$_Payment['accountname']	= $resultMap['VACT_Name'];		//예금주
						$_Payment['accountno']		= $resultMap['VACT_Num'];			//계좌번호
						$_Payment['account_dueDate'] = $resultMap['VACT_Date']." ".$resultMap['VACT_Time'];		//입금마감일
					}else if($order_info->pay_method == "PN"){
						# 계좌이체
						$_Payment['bankkind']		= $resultMap['ACCT_BankCode'];	//은행코드
						$_Payment['accountname']	= $resultMap['buyerName'];	    //구매자
					}else if($order_info->pay_method == "PC"){
						$_Payment['cardname']        = $resultMap['CARD_PurchaseName']; // 결제카드
						$_Payment['financename']     = $resultMap['CARD_PurchaseName']; // 결제카드사	//2020-03-19 정나혜 추가
						$_Payment['financename_num'] = $resultMap['CARD_Num'];          // 결제카드번호
						$_Payment['cardinterest']    = $resultMap['CARD_Interest'];     // 카드할부여부("1"이면 무이지할부)
						$_Payment['cardquota']       = $resultMap['CARD_Quota'];        // 카드할부기간
					}else{
						$_Payment['bankkind']		= $resultMap['VACT_BankCode'];	//은행코드
					}

					$_Payment['orderid']		= $_REQUEST["orderNumber"];		//주문번호
					$_Payment['paymethod']	= $order_info->pay_method;		//결제종류
					$_Payment['ttno']			= $resultMap['tid'];				//거래번호
					$_Payment['pgname']		= "inicis";						//PG사 종류
					$_Payment['es_check']		= $oper_info['pay_escrow'];		//에스크로 사용여부
					$_Payment['es_stats']		= "IN";
					$_Payment['tprice']		= $resultMap['TotPrice']; //결제금액
					foreach($_Payment as $key => $value){	$logs .="$key : $value\r";	}
					make_log(LOG_PATH."inicis_log.log","\r----------[".date("Ymd")."]order_update_vir.php start--------\r".$logs."\r-------order_update_vir.php end--------\r");
					...
					*/
					...
					$resultMap["resultMsg"] = $netcancelResultString;
                }
            } else {
               ...
               // echo "<pre>" . var_dump($_REQUEST) . "</pre>";
				$resultMap["resultCode"] = $_REQUEST["resultCode"];
				$resultMap["resultMsg"]  = $_REQUEST["resultMsg"];
            }
?>
		<html>
		<body>
		<form name="frm" action="/shop/order.php" method="post">
		<input type="hidden" name="ptype" value="ok">
		<input type="hidden" name="orderid" value="<?=$order_info->orderid?>">
		<input type="hidden" name="rescode" value="<?=$resultMap["resultCode"]?>">
		<input type="hidden" name="resmsg" value="<?=$resultMap["resultMsg"]?>">
