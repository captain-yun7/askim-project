...
$cut = explode("/",$PHP_SELF);
if($cut[1] == "m" || mobile_check()){
?>
<script> document.cookie = 'PM=M; expires=""; path=/';</script>
<?}else{?>
<script> document.cookie = 'PM=PC; expires=""; path=/';</script>
<?}?>
<script> document.cookie = 'prev_page=<?=$prev?>; expires=""; path=/';</script>
...
<script type="text/javascript">
	...
	/*
	//IE. 8~9 버전에서 console.log 스크립트 오류로 인해 아래와 같은 코드 추가
	var console = console || {
		log:function(){},
		warn:function(){},
		error:function(){}
	...
	var kakao_api_key = "<?=$kakao_api_key?>";
	Kakao.init(kakao_api_key);

	function getKakaotalkUserProfile(){
		var kakao_login_location = "<?=$kakao_login_location?>";
		Kakao.API.request({
			url: '/v1/user/me',
					...
					success: function (data) {
						...
						if (data == "ok") {
							...
							if(confirm("선택하신 계정정보로 가입된 ID가 존재합니다.\n해당ID와 연결하시겠습니까?")){
								...
								document.cookie = "kakao_username="+res.properties.nickname+"; path=/";
								document.cookie = "kakao_id="+res.id+"; path=/";
								document.cookie = "kakao_email="+user_Email+"; path=/";
								document.cookie = "login_Type="+login_Type+"; path=/";
								document.cookie = "sns_login="+sns_Login+"; path=/";
							...
							var name = encodeURI(user_Name);
							if (confirm("카카오 계정으로 가입하시겠습니까?\n카카오 계정으로 가입시 추가정보를 입력하셔야 합니다.")) {
								var $form = $("<form></form>");
								if("<?=$cut[1]?>"=="m"){
	...
	function createKakaotalkLogout(){
		...
		logoutBtn.click(function(){
			...
			$("#kakao-profile").text("");
			document.cookie = "kakao_username=; path=/";
			document.cookie = "kakao_id=; path=/";
		});
	...
	function kakaologout(){
		Kakao.Auth.logout(function() { location.href = kakao_login_location });
		$("#kakao-profile").text("");
		document.cookie = "kakao_username=; path=/";
		document.cookie = "kakao_id=; path=/";
	}
...
function naver_login() {
	...
	window.open(uurl,'네이버 로그인','width=600, height=600, scrollbars=yes');
	}
...
<script>
	function twitter_login(){
		...
		window.open(url,'트위터 로그인','width=600, height=300');
		}
	}
</script>
...
for($ii=0; $ii<count($sns_login_method)-1; $ii++){
	if($sns_login_method[$ii] == "FB"){
		$facebook_btn = "<a href='javascript:checkFacebookLogin();' class='btnFace'>페이스북 ID 회원가입</a>";
	}
	if($sns_login_method[$ii] == "TT"){
		$twitter_btn  = "<img src='$skin_dir/image/login_twitter_bt.gif' onclick='twitter_login()' style='cursor:pointer'>";
	}
	if($sns_login_method[$ii] == "KT"){
		$kakaotalk_btn  = "<a href=\"javascript:".$kakao_link."\" class='btnKakao'>카카오톡 ID 회원가입</a>";
	}
	if($sns_login_method[$ii] == "NH"){
		$naver_btn  = "<a href='javascript:naver_login();' class='btnNaver'>네이버 ID 회원가입</a>";
	}
	if($sns_login_method[$ii] == "GG"){
		$google_btn  = "<a class='btnGoogle customGPlusSignIn'  id='customBtn'>구글로 가입하기</a><script>startApp();</script>";
	}
}
...
include $_SERVER['DOCUMENT_ROOT'].'/'.$skin_dir.'/join_list.php';
?>