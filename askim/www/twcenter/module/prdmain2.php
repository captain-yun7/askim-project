...
$sql = "select wp.*, wc.purl, wc.catcode
				from wiz_product2 as wp left join wiz_cprelation2 as wcp on wp.prdcode = wcp.prdcode
				left join wiz_category2 as wc on wcp.catcode = wc.catcode
...
while($row = sql_fetch_arr($result)){
	...
	$purl = $row['purl'];
	if($purl == "") {
		$p_sql = "select purl from wiz_category2 where catuse != 'N' and purl != '' order by catcode asc limit 1";
		$p_row = sql_fetch($p_sql);
		$purl = $p_row['purl'];
	}

	$conprice = "";
	if(isset($row['conprice'], $row['sellprice']) && $row['conprice'] > $row['sellprice']){
	...
	if(!empty($row['strprice'])) {
		...
		$sellprice = $row['strprice'];
			...
			if ( isset($row['sellprice']) && is_numeric($row['sellprice'])) {
	...
	// 상품아이콘
	$sp_img = "";
	if(isset($row['popular']) && $row['popular'] == "Y") 	$sp_img .= "<img src='/twcenter/images/icon_hit.gif'>&nbsp;";
	if(isset($row['recom']) && $row['recom'] == "Y") 		$sp_img .= "<img src='/twcenter/images/icon_rec.gif'>&nbsp;";
	if(isset($row['newc']) && $row['newc'] == "Y") 		$sp_img .= "<img src='/twcenter/images/icon_new.gif'>&nbsp;";
	if(isset($row['sale']) && $row['sale'] == "Y") 		$sp_img .= "<img src='/twcenter/images/icon_sale.gif'>&nbsp;";
	if(isset($row['best']) && $row['best'] == "Y") 		$sp_img .= "<img src='/twcenter/images/icon_best.gif'>&nbsp;";
	if((isset($row['shortage']) && $row['shortage'] == "Y") || (isset($row['shortage'], $row['stock'])  && $row['shortage'] == "S" && $row['stock'] <= 0)) $sp_img .= "<img src='/twcenter/images/icon_not.gif'>&nbsp;";
	...
	$coupon_img = "";
	if(
	isset($row['coupon_use'], $row['coupon_sdate'], $row['coupon_edate'], $row['coupon_limit'], $row['coupon_amount'] ) &&
	$row['coupon_use'] == "Y" &&
	...
	if(file_exists(WIZHOME_PATH."/data/product2/".$row['prdimg_R']) && !empty($row['prdimg_R'])) $prdimg = "/twcenter/data/product2/".$row['prdimg_R'];		// img
	...
	$shortexp = cut_str($row['shortexp'],$prdname_len);
	$prdnum = isset($row['prdcode']) ? $row['prdcode'] : '';
