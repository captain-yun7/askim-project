...
$tmp_catcode_sub = substr($pcategory,0,2); //카테고리
...
if($maintype == "mainpro") $maintype_sql = " and wp.mainpro='Y' ";
if($maintype == "mainpro_m") $maintype_sql = " and wp.mainpro_m='Y' ";

//2024-03-25 정나혜 [모바일 진열] 사용시에만 모바일진열 값에 따라 노출하도록 수정
if($site_info['mobile_use'] == 'Y' && $site_info['mobile_show_use'] == "Y"){
	$show_sql = " wp.mobileShow != 'N' ";

}else{
	$show_sql = " wp.showset != 'N' ";
}
...
$sql = "select wp.*, wc.purl
				...
				where $show_sql and wc.catuse != 'N' $maintype_sql $pcategory_sql
...
while($row = sql_fetch_arr($result)){
	...
	if($row['conprice'] > $row['sellprice']){
		$conprice = "<s>".number_format($row['conprice'])."</s>원 <br/> ";
	}
	...
	if($row['newc'] == "Y") 		$sp_img .= "<img src='/twcenter/images/icon_new.gif'>&nbsp;";
	...
	if($purl != "") $prdlink = "/m/sub/prdview.php?ptype=view&prdcode=".$row['prdcode']."&catcode=".$pcategory;
	else $prdlink = "javascript:alert('상품 페이지가 제대로 설정되있지 않습니다.');";
	$prdname = cut_str($row['prdname'],$prdname_len);

    $stortexp = cut_str($row['stortexp'],26);

	$body_tmp = str_replace("{PRDNAME}",$prdname,$body_tmp);
	$body_tmp = str_replace("{STORTEXP}",$stortexp,$body_tmp); //상품요약설명
	$body_tmp = str_replace("{PRDPRICE}",$prdprice,$body_tmp);
	$body_tmp = str_replace("{PRDCODE}",$row['prdcode'],$body_tmp);
