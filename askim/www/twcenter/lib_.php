...
function print_pagelist($page, $list_amount, $page_count, $param, $page_type = ""){
   ...
   if($param != "") $param = "&".$param;
	...
	if($epage > 0) {
		...
		echo "                     <li><a href='$PHP_SELF?$page_name=1$param'>처음</a></li>\n";
		echo "                     <li><a href='$PHP_SELF?$page_name=$ppage$param'>이전</a></li>\n";
		...
		echo "                     <li><a href='$PHP_SELF?$page_name=$npage$param'>다음</a></li>\n";
		echo "                     <li><a href='$PHP_SELF?$page_name=$page_count$param'>맨끝</a></li>\n";
		echo "                   </ul></div>\n";
...
function ContentImgResizeCheck($content,$code) {
	...
	//	print_r($img_p);
		...
		if(strpos($match[0][$key], $p_dir)) {
			if($get_width)
				$img_url  = '<img src="'.WAY_HOST.$img_path.'" width="'.$get_width.'" height="'.$get_height.'">';
			else
				$img_url  = '<img src="'.WAY_HOST.$img_path.'">';
		} else {
			$img_url  = '<img src="'.$get_src.'">';
		}
	...
	//	echo preg_match("/\.(gif|jpg|jpeg|png)$/i", $fname)."<br>";
		...
		if(strpos($match[0][$key], $p_dir) && preg_match("/\.(gif|jpg|jpeg|png)$/i", $fname)) {
			$img_link = '<a href=javascript:bbsviewImg("'.$code.'","'.$img_enc.'","edit")>'.$img_url.'</a>';
		} else {
			$img_link = $img_url;
		}
...
function get_spam_check(){
	...
	if (function_exists("imagecreate")) {	// 이미지 생성이 가능한 경우 자동등록체크코드를 이미지로 생성
	  $norobot_img = "<span><img id='norobot_key_img' src='/twcenter/bbs/norobot_image.php?ss_norobot_key=$norobot_key' border='0' style='height:33px;padding:5px 0 4px 0; background:#242b38; border: 1px solid #343d4f;' align='absmiddle' alt='자동등록방지번호'></span>";
	...
	$spam_check = $norobot_img." <input type='text' name='vcode' id='vcode' class='input' style='height:44px; line-height:44px; padding:0 5px; font-size:16px;' title='자동등록방지 입력' /> ".$norobot_msg;
...
function xss_clean($str, $get_html = true, $chk = true) {
	if(!empty($str)) {
		// 동영상업로드시 embed때문에 일단 삭제함
		$param_chk = array("onstop","layer","javascript","eval","onactivae","onfocusinapplet","document","onclick","onkeydownxml","create","onbeforecut","onkeyuplink","binding","ondeactivate","onloadscript","msgbox","ondragend","onbounceobject","ondragleave","onmovestartframe","applet","ondragstart","onmouseoutilayer","onerror","onmouseupbgsound","onabortbase","onstart","onfocus","onmovestartonmove","onrowexit","onunload","onsubmitinnerHTML","onpaste","ondblclick","vpscriptcharset","onresize","ondrag","expressionstring","onselect","ondragenter","onchangeappend","onscroll","ondragover","metaalert","ondrop","voidrefresh","oncopy","oncutilayer","blink","onfinish","framesetcookie","onreset","onselectstart","script","join","declare","column_name","table_name","openrowset","substr","substring","xp_","sysobjects","syscolumns","onload","alert");

		if($chk) {
			for($i=0;$i<count($param_chk);$i++){
				$chk_str = strtolower($str);
				$check = strpos($chk_str, $param_chk[$i]);
				if ($check === false) {
...
function get_bbs_no($data) {
	...
	$idx         = sqlfilter($data['idx']);
	$page        = sqlfilter($data['page']);
...
function Exe_payment($payment){
	...
	@make_log($_SERVER['DOCUMENT_ROOT']."/twcenter/data/order_update_log.txt","--START".date("Y/m/d H:i:s",time())."---------\r".print_r($payment, true).$status."\r".$orderid."\r".$financecode."\r".$accountnumber."\r".$tno."\r".$esw_check."\r".$esw_stats."\r".$totalprice."\r".$paymentkind."\r".$pay_method."\r");
	...
	if(!$result) {
		@make_log($_SERVER['DOCUMENT_ROOT']."/twcenter/data/order_update_log.txt","\r------------order update error-------------------\r".$sql."\r\r");
	} else {
		$isDBOK = true;
	}
	@make_log($_SERVER['DOCUMENT_ROOT']."/twcenter/data/order_update_log.txt","\r".$sql."\r---------------------------End----------------------------------\r");
...
function send_alimtalk($templateCode,$talk_info){
	...
	$company_name = $site_info['alimtalk_name'];
	...
	if($rw1['remain_cnt'] > 0 && $alimtalk_use == 'Y' && $rw1['cnt'] > 0) {
		...
		$return_sn        = $json_response[0]['sn'];
		$return_custMsgSn = $json_response[0]['custMsgSn'];
		$return_code      = $json_response[0]['code'];
		$return_altCode   = $json_response[0]['altCode'];
		$return_altMsg    = $json_response[0]['altMsg'];
		$return_sndDtm    = $json_response[0]['sndDtm'];
		$return_rcptDtm   = $json_response[0]['rcptDtm'];
