...
if($mode == "update"){
	...
	$sql = "SELECT * FROM {$this_tbl} WHERE (idx = '$idx' OR (id = '$id' AND id != ''))";
	...
	if($site_info['login_pw_chg'] == "Y") {
		...
		if($pw_update >= $site_info['login_pwchg_day']) {
			$alt_msg = "<font color='red'>* 비밀번호를 주기적으로 변경해주세요. (비밀번호 변경일 ".$pw_update.")</font>";
		} else {
			$alt_msg = "";
		}
	} else {
	...
	// 적립금
	$sql = "SELECT SUM(reserve) OR reserve FROM wiz_reserve WHERE memid = '".$my_info['id']."'";
	$result = query($sql) or error("sql error");
	$row = sql_fetch_obj($result);
	$reserve = $row->reserve;
}
...
function createObject($no,$type,$size,$flist, $value){
	...
	if($type == "select") $finput = "<select id=\"".$fname."_0\" name=\"fname[".$no."][]\" class='select'><option value=''>--</option>";
...
<?php } else { ?>
	...
	if(hphone0 != undefined || hphone1 != undefined || hphone2 != undefined) {
		var str_hphone = hphone0 + "-" + hphone1 + "-" + hphone2;
		if(!check_Hphone2(str_hphone, "올바른 휴대전화번호를 입력하세요.")) return false;
	}
...
<?php } else { ?>
	...
	if(comtel0 != undefined || comtel1 != undefined || comtel2 != undefined) {
		var comtel  = [];
		var comtel_input = $("input[name='comtel[]']");
		$(comtel_input).each(function(i){
			var tmp_comtel = $("#comtel" + i).val();
...
function ReleaseSleep(idx){
	if(idx == ""){
		alert("휴면해제를 위한 필수값이 존재하지 않습니다.");
		return false;
	} else {
		var url = "member_save.php?idx=" + idx + "&mode=daccountChg";
...
-->
        ...
        <tr>
          ...
          <td width="2"></td>
          <td valign="bottom" class="tit_alt">휴면회원을 검색/수정 관리합니다.</td>
        </tr>
      </table>
      ...
      <input type="hidden" name="inputType" value="dormancy">

      	  <?
	  if($wiz_admin['id'] != "" && $idx == ""){
			$idxresult = query("select idx from {$this_tbl} where id = '$id'") or error('에러');
...
?>
				<td class="t_value"><?=number_format($total_price)?>원 <input type="button" value="상세보기" class="base_btm reg" onclick="orderList('<?=$my_info['id']?>','<?=$my_info['name']?>')"></td>
				<td class="t_name">적립금</td>
				<td class="t_value"><?=number_format($reserve)?>원 <input type="button" value="상세보기" class="base_btm reg" onclick="reserveList('<?=$my_info['id']?>','<?=$my_info['name']?>')"></td>
<?
				}else{
?>
				<td class="t_value" colspan="3"><?=number_format($total_price)?>원 <input type="button" value="상세보기" class="base_btm reg" onclick="orderList('<?=$my_info['id']?>','<?=$my_info['name']?>')"></td>
<?
			}
?>
			</tr>
			<? } ?>
              <tr>
                <td class="t_name">휴면전환일</td>
                <td colspan="3" class="t_value">
                  <strong><font color="#A90329"><?php echo $my_info['dchange_date'] ?></font></strong>
                </td>
              </tr>
              ...
              <tr>
                ...
                <td colspan="3" class="t_value">
                  <? list($tphone1, $tphone2, $tphone3) = explode("-",$my_info['tphone']); ?>
					<input type="text" name="tphone[]" value="<?=$tphone1?>" id="tphone0" size="5" maxlength="3" class="input" Onlynum="true" > - 
					<input type="text" name="tphone[]" value="<?=$tphone2?>" id="tphone1" size="5" maxlength="4" class="input" Onlynum="true" > - 
					<input type="text" name="tphone[]" value="<?=$tphone3?>" id="tphone2" size="5" maxlength="4" class="input" Onlynum="true" >
                </td>
              </tr>
              <? } ?>
              ...
              <tr>
                ...
                <td colspan="3" class="t_value">
					...
					<input type="text" name="hphone[]" id="id_hphone1" value="<?=$hphone1?>" id="id_hphone1" size="5" maxlength="3" class="input" Onlynum="true" > - 
					<input type="text" name="hphone[]" id="id_hphone2" value="<?=$hphone2?>" id="id_hphone2" size="5" maxlength="4" class="input" Onlynum="true" > - 
					<input type="text" name="hphone[]" id="id_hphone3" value="<?=$hphone3?>" id="id_hphone3" size="5" maxlength="4" class="input" Onlynum="true"  <?php echo $hphone_chk ?>> 
					<?php if($info_use['resms'] == true){ ?>
					<span style="vertical-align: middle"><input type="checkbox" name="resms" value="Y" <? if($my_info['resms'] == "Y") echo "checked"; ?>> SMS수신에 동의합니다.</span>
					<?php } ?>
                </td>
              </tr>
              <? } ?>
			...
			<? if($info_use['comtel'] == true){ ?>
              <tr>
                ...
                <td colspan="3" class="t_value">
					...
					<input type="text" name="comtel[]" value="<?=$comtel1?>" id="comtel0" size="5" maxlength="3" class="input" Onlynum="true" > - 
					<input type="text" name="comtel[]" value="<?=$comtel2?>" id="comtel1" size="5" maxlength="4" class="input" Onlynum="true" > - 
					<input type="text" name="comtel[]" value="<?=$comtel3?>" id="comtel2" size="5" maxlength="4" class="input" Onlynum="true" > 
                </td>
              </tr>
              <? } ?>
              ...
              <tr>
                ...
                <td colspan="3" class="t_value">
					...
					<span style="vertical-align: middle"><input type="checkbox" name="reemail" value="Y" <? if($my_info['reemail'] == "Y") echo "checked"; ?>> 이메일수신에 동의합니다.</span>
					<?php } ?>
                </td>
              </tr>
              <? } ?>
              ...
              <tr>
                ...
                <td colspan="3" class="t_value" colspan="3">
					<input name="post" type="text" value="<?=$my_info['post']?>" size="7" class="input" style="text-align:center"> 
					<input type="button" value="우편번호검색" class="base_btn2" onClick="searchZip('');"><span class="tip_br5"></span>
					<input name="address1" type="text" value="<?=$my_info['address1']?>" class="input input580"><span class="tip_br5"></span>
					<input name="address2" type="text" value="<?=$my_info['address2']?>" class="input input580">
                </td>
              </tr>
              <? } ?>
              ...
              <tr>
                ...
                <td class="t_value" colspan="3">
                  ...
                  <input name="birthday[]" value="<?=$birthday1?>" type="text" class="input" id="birthday0" size="4" maxlength="4" Onlynum="true">년
                  <input name="birthday[]" value="<?=$birthday2?>" type="text" class="input" id="birthday1" size="2" maxlength="2" Onlynum="true">월
                  <input name="birthday[]" value="<?=$birthday3?>" type="text" class="input" id="birthday2" size="2" maxlength="2" Onlynum="true">일 (
                  <span style="vertical-align: middle"><input type="radio" name="bgubun" value="양력" <? if($my_info['bgubun'] == "양력") echo "checked"; ?>></span>양력
                  <span style="vertical-align: middle"><input type="radio" name="bgubun" value="음력" <? if($my_info['bgubun'] == "음력") echo "checked"; ?>></span>음력 )
                </td>
              </tr>
              <? } ?>
              ...
              <tr>
                ...
                <td colspan="3" class="t_value">
                  <span style="vertical-align: middle"><input type="radio" name="marriage" value="미혼" <? if($my_info['marriage'] == "미혼") echo "checked"; ?>></span>미혼
                  <span style="vertical-align: middle"><input type="radio" name="marriage" value="기혼" <? if($my_info['marriage'] == "기혼") echo "checked"; ?>></span>기혼
                </td>
              </tr>
              <? } ?>
              ...
              <tr>
                ...
                <td colspan="3" class="t_value">
                  ...
                  <input name="memorial[]" value="<?=$memorial1?>" id="memorial0" type="text" size="4" maxlength="4" class="input" Onlynum="true">년
                  <input name="memorial[]" value="<?=$memorial2?>" id="memorial1" type="text" size="2" maxlength="2" class="input" Onlynum="true">월
                  <input name="memorial[]" value="<?=$memorial3?>" id="memorial2" type="text" size="2" maxlength="2" class="input" Onlynum="true">일
                </td>
              </tr>
              <? } ?>
              ...
              <tr>
                ...
                <td class="t_value" colspan="3">
                	<span style="vertical-align: middle"><input name="car" type="radio" value="소유" <? if($my_info['car'] == "소유") echo "checked"; ?>></span>소유
					<span style="vertical-align: middle"><input name="car" type="radio" value="미소유" <? if($my_info['car'] == "미소유") echo "checked"; ?>></span>미소유
                </td>
              </tr>
              <? } ?>
      ...
      <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
          <td align="center">
			...
			<input type="button" value="휴면해제" class="base_btn purple" onClick="ReleaseSleep(<?php echo $idx ?>)">&nbsp;
			<input type="button" value="목록" class="base_btn gray" onClick="document.location='member_dormant_account_list.php?<?=$param?>';">
        </tr>
      </table>
	  </form>
