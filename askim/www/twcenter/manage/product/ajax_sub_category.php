...
function reorder(){
	if(confirm("상품 정렬 순서가 변경되지 않는 문제 발생시에만 사용 바랍니다.\n상품분류를 재정렬하시겠습니까?")) {
		location.href="category_save.php?mode=reorder";
	}
}
</script>
...
<div id="treeControl" style="padding: 5px 0">
	<img src="/twcenter/manage/image/tree/ServerMag_Etc_Root.gif" style="vertical-align:middle; padding:0 0 0 5px"> <a href="javascript:moveCode2('00000000','<?=$menucode?>')" class="a_cat_item"><span class="fontSize">최상위</span></a><br>
	...
	<li id="li_<?php echo $row->catcode; ?>">
		<a href="javascript:;" onclick="moveCode('<?=$row->catcode?>','<?=$row->depthno?>','<?=$row->priorno01?>','<?=$menucode?>')" class="a_cat_item"><span class="folder"><?=$catname?> <?=$cnt02?></span></a>
		<ul>
		<?php
		...
		while($row2 = sql_fetch_obj($result2)) {
			...
			//$default_folder  = ($cnt03 == 0) ? "file" : "folder";
			if($row2->catcode == $catcode) $row2->catname = "<span class='clickover'>".$row2->catname."</span>";

		?>
			<li id="li_<?php echo $row2->catcode; ?>">
				<a href="javascript:;" onclick="moveCode('<?php echo $row2->catcode ?>','<?php echo $row2->depthno ?>','<?php echo $row2->priorno02 ?>','<?php echo $menucode ?>')" class="a_cat_item"><span class="folder"><?=$row2->catname?> <?=$cnt03?></span></a>
				<ul>
				<?php
				...
				while($row3 = sql_fetch_obj($result3)) {
					...
					//$default_folder  = ($cnt04 == 0) ? "file" : "folder";
					if($row3->catcode == $catcode) $row3->catname = "<span class='clickover'>".$row3->catname."</span>";

				?>
					<li id="li_<?php echo $row3->catcode; ?>">
						<a href="javascript:;" onclick="moveCode('<?php echo $row3->catcode ?>','<?php echo $row3->depthno ?>','<?php echo $row3->priorno03 ?>','<?php echo $menucode ?>')" class="a_cat_item"><span class="folder"><?=$row3->catname?> <?=$cnt04?></span></a>
						<ul>
						<?php
						...
						?>
							<li>
								<a href="javascript:;" onclick="moveCode('<?php echo $row4->catcode ?>','<?php echo $row4->depthno ?>','<?php echo $row4->priorno04 ?>','<?php echo $menucode ?>')" class="a_cat_item"><span class="file"><?=$row4->catname?></span></a>
							</li>
						<?php
						}
						?>
