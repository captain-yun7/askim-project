...
$params[] = "t_month=".$t_month;
$params[] = "t_date2=".$t_date2;
...
for($i=0; $i<=23; $i++){
	...
	$g_time_play   = (strlen($i)==1) ? "0".$i : $i;
	$g_start_time  = $t_year."-".$t_month."-".$t_date2." ".$g_time_play.$s_time;
	$g_end_time    = $t_year."-".$t_month."-".$t_date2." ".$g_time_play.$e_time;
	...
	${"g_total_price_OR_".$i} = (int)(${"g_pc_total_price_OR_".$i} + ${"g_mo_total_price_OR_".$i});
	...
	${"conn_pg_sql_".$i} = "
		...
		where 1
			...
			$t_year_month_day_sql $t_paymethod_sql and (status='OR' or status='OY' or status='DR' or status='DI' or status='DC') and (pay_method='PC' or pay_method='PN' or pay_method='PV' or pay_method='KK')
	...
	${"conn_ac_sql_".$i} = "
		...
		where 1
			...
			$t_year_month_day_sql $t_paymethod_sql and (status='OR' or status='OY' or status='DR' or status='DI' or status='DC') and pay_method='PB'
...
function excelDown(){
	location.href="_total_time_analy_excel.php?<?php echo $param; ?>";
}
		...
		<table border="0" cellspacing="0" cellpadding="2">
		  <tr>
		    ...
		    <td valign="bottom" class="tit">시간별매출</td>
		    <td width="2"></td>
		    <td valign="bottom" class="tit_alt">결제수단별, 시간별 통계분석</td>
		  </tr>
		</table>
		<br>
		...
		<table width="100%" cellspacing="1" cellpadding="3" border="0" class="t_style">
			<tr>
				<td width="12%" class="t_name">&nbsp; 기 간</td>
				<td width="38%" class="t_value">
					...
					<select name="t_month" class="select">
						<?
						for($i=1; $i<=12; $i++){
							$month_str = (strlen($i)==1) ? "0".$i : $i;
						?>
						<option value="<?=$month_str?>" <? if($month_str==$t_month) echo "selected";?>><?=$month_str?>월</option>
						<?
						}
						?>
					</select>&nbsp;
					<select name="t_date2" class="select">
						<?
						for($i=1; $i<=$last_month_day; $i++){
							$date_str = (strlen($i)==1) ? "0".$i : $i;
						?>
						<option value="<?=$date_str?>" <? if($date_str==$t_date2) echo "selected";?>><?=$date_str?>일</option>
						<?
						}
						?>
					</select>
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="2">
			<tr>
				<td class="tit_sub"><img src="../image/ics_tit.gif" align="absmiddle"> <?=$t_year?>년 <?=$t_month?>월 <?=$t_date2?>일 총 매출통계</td>
			</tr>
		</table>
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="2">
			<tr>
				<td class="tit_sub"><img src="../image/ics_tit.gif" align="absmiddle"> <?=$t_year?>년 <?=$t_month?>월 <?=$t_date2?>일 시간별 매출통계</td>
