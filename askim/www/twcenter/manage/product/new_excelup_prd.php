				...
				<tr>
					...
					<td>&nbsp;
						<span class="filebox preview-image">
							...
							<input type="file" name="upfile" id="input-file" class="upload-hidden"> <a href="del_excel_prd.php"><img src="../image/orderlist_down.gif" border="0" align="absmiddle"></a>
						</span>
					</td>
				</tr>
			</table>
...
<div class="helpTip">
	...
	<div class="content">
		<div class="explain">
		1. <a href="del_excel_prd.php"><b><font color="black">[주문내역다운로드]</font></b></a>를 클릭하여 주문내역을 다운로드합니다.<br>
	...
	if(is_uploaded_file($UpFile["tmp_name"])) {
		...
		for ($i = 3 ; $i <= $maxRow ; $i++) {
			...
			*/
			...
			$orderid           = $objWorksheet->getCell('A' . $i)->getValue();
			$prdcode           = $objWorksheet->getCell('B' . $i)->getValue();
			$totalprice        = $objWorksheet->getCell('D' . $i)->getValue();
			$delcom            = $objWorksheet->getCell('E' . $i)->getValue();
			$delnum            = $objWorksheet->getCell('F' . $i)->getValue();
			$deldate           = $objWorksheet->getCell('G' . $i)->getValue();
			...
			*/
			...
			$sql = "update wiz_basket
					set del_num = '$delnum', deliver_date = '$deldate', del_com = '$deli_join'
					where orderid = '$orderid' and prdcode = '$prdcode' and prdprice = '$totalprice' ";
