...
//2021-05-12 엑셀다운로드를 위한 결제수단 파라미터 추가 정나혜
if(isset($t_pay_method)    && $t_pay_method) {
	for($i=0; $i<count($t_pay_method); $i++){
		$tmp_param[] .= "t_pay_method[]=".$t_pay_method[$i];
	}
}
...
<?php
$sql = "
	select count(distinct wo.orderid) AS total 
	  from wiz_order wo
	 where orderid !='' 
	   $search_query
 ...
 * 결제완료시에만 가능
 */
$_sql2 = "select * from wiz_order where orderid !='' and status = 'OY' ";
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
	...
	while(($row = sql_fetch_obj($result)) && $rows){
		...
		if(!strcmp($row->escrow_check, "Y")) $escrow_check = "<span class=ascro_icon>에스크로</span>";
		else  $escrow_check = "";
	...
	<tr onmouseover="this.style.backgroundColor='#e8f3f7'" onmouseout="this.style.backgroundColor='#ffffff'">
		<td align="center" class="t_line"><input type="checkbox" name="select_checkbox"></td>
		<td align="center" class="t_line"><a href="order_info.php?orderid=<?=$row->orderid?>&page=<?=$page?>&<?=$param?>"><?=$_orderid?></a></td>
		...
		<td align="center" class="t_line">
			<table cellpadding="1" width="100%">
				<tr>
					<td bgcolor="<?=$stacolor?>" align="center" class="t_line2">
						<select name="chg_status" style="width:150px" class="select2">
					...
					<td><input type="submit" value="적용" class="base_btm blue2"></td>
				</tr>
			</table>
		</td>
		<td align="center" class="t_line">
			<input type="button" value="상세보기" class="base_btm reg" onClick="document.location='order_info.php?orderid=<?=$row->orderid?>&page=<?=$page?>&<?=$param?>'">
		</td>
	</tr>
	</form>
	<?
