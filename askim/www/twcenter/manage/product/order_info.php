...
function sendEmail(name,email){
	var url = "../message/mail_popup.php?seluser=" + name + ":" + email;
...
function sendSms(sms_u,hphone,status){
	...
	var url = "../message/sms_popup.php?sms_u=" + sms_u + "&seluser=" + hphone;
...
function delivery_sel(idx,orderid) {
	...
	document.location = "order_save.php?mode=deliverySel&idx="+idx+"&orderid="+orderid+"&del_com="+del_com+"&<?=$menucodeParam?>";
}
...
//2023-12-28 체크박스 전체선택
function selectAll(){

	var i;

	for(i=0;i<document.forms.length;i++){
		if(document.forms[i].didx != null){
...
//2023-12-28 제품별 배송정보 일괄 입력 체크
function chgdelinfo(){
	
	//var del_com = $("#del_com option:selected").text();
	var del_com = $("#del_com").val();
		...
		if(document.forms[i].didx != null){
			if(document.forms[i].select_checkbox){
				if(document.forms[i].select_checkbox.checked)
					selorder = selorder + document.forms[i].didx.value + "|";
			}
		}
	}

	if(selorder == ""){
		alert("배송정보를 저장할 주문을 선택하지 않았습니다.");
		return;
	}else if(del_com == ""){
		alert("배송업체를 선택하지 않았습니다.");
		$("#del_com").focus();
		return;
	/*
	}else if(del_num == ""){
	...
	*/
		...
		if(confirm("선택한 주문의 배송정보를 변경하시겠습니까?")){
			document.location = "order_save.php?mode=chgDelInfo&orderid=<?=$orderid?>&selorder=" + selorder + "&del_com="+del_com+"&del_num="+del_num+"&del_date="+del_date+"&<?=$menucodeParam?>";
		}else{
			return;
		}
	}
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
	<?php
	/* 2023-12-28 상품별 배송정보 입력 > 체크박스를 위한 폼 추가 */
	?>
	<form>
	<tr><td class="t_rd" colspan="20"></td></tr>
	<tr class="t_th">
		<?php
		//2023-12-28 상품별 배송정보 입력 > 일괄선택을 위한 체크박스
		if($oper_info['deliveryType'] == "P"){
		?>
		...
		?>
		<th width="8%">합 계<!--br>(실결제액)--></th>
		<th width="8%">할인내역</th>
		<th width="7%">취 소</th>
	</tr>
	</form>
	<?php
	/* 2023-12-28 상품별 배송정보 입력 > 체크박스를 위한 폼 추가 (닫기) */
	?>
	<tr><td class="t_rd" colspan="20"></td></tr>
	<?
	...
	while($row = sql_fetch_obj($result)){
		...
		$c_sql = "
			...
			select wc.purl
			from 
				wiz_product as wp left join wiz_cprelation as wcp on wp.prdcode = wcp.prdcode
		...
		$prd_pay_price = prd_pay_price($row);

	?>
	<?php
	/* 2023-12-28 상품별 배송정보 입력 > 체크박스를 위한 폼 추가 */
	?>
	<form name="basket_frm<?=$no?>">
	<tr bgcolor="#FFFFFF" height="55">
		<? 
		//2023-12-28 상품별 배송정보 입력 > 일괄선택을 위한 체크박스
		if($oper_info['deliveryType'] == "P"){
		?>
		<input type="hidden" name="didx"  value="<?=$row->idx?>">
		 ...
		 */
		if($oper_info['deliveryType'] == "P"){
			 if(empty($row->del_com)) $align = "center" ; else $align = "left"
		?>
		<td align="<?=$align?>">
			<? if(empty($row->del_com)){ ?>
			<img src="../image/btn_deli_collect.gif" style="cursor:pointer" onclick="delivery('<?=$orderid?>',<?=$row->idx?>)">
			<? } else { ?>
		...
		?>
		<td align="center"><?=number_format($prd_d_price)?>원 <?//if($prd_pay_price != $row->prdprice) echo "<BR>(".@number_format($prd_pay_price)."원)";?></td>
		<td align="center">
		<?
			if($row->discount_price != '' && $row->discount_price != 0) echo "- 회원할인 : ".@number_format($row->discount_price)."원<BR>";
			if($row->coupon_use != '' && $row->coupon_use != 0) echo "- 쿠폰사용 : ".@number_format($row->coupon_use)."원<BR>";
			//if($row->reserve_use != '' && $row->reserve_use != 0) echo "- 적립금사용 : ".@number_format($row->reserve_use)."원<BR>";
		?>			
		</td>
		<td align="center">
		<?
	...
	if($order_info->discount_price > 0){
		...
		$discount_msg = " <span class=\"price_color2\">－ 회원할인 금액 (<span class=\"tbold_r\">".number_format($order_info->discount_price)."</span> 원)</span>";
		$_drow = 1;
	}
	// 적립금 사용
	if($order_info->reserve_use > 0){
		...
		$reserve_msg = " <span class=\"price_color2\">－ 적립금 사용 (<span class=\"tbold_r\">".number_format($order_info->reserve_use)."</span> 원)</span>";
		$_rrow = 1;
	}
	...
	if($order_info->coupon_use > 0){
		...
		$coupon_msg = " <span class=\"price_color2\">－ 쿠폰사용 (<span class=\"tbold_r\">".number_format($order_info->coupon_use)."</span> 원)</span>";
		$_crow = 1;
	}

	// 배송비
	$deliver_price = " <span class=\"price_color1\">＋ 배송비 (<span class=\"tbold_b\">".number_format($order_info->deliver_price)."</span> 원)</span>";
	...
	<tr class="t_name">
		<td colspan="12" style="padding:5px">
			&nbsp; 선택한 상품을 &nbsp;
			<select id="del_com" class="select">
			...
			?>
				<option value="<?=$del_code?>"><?=$_delivery['del_com']?></option>
			<?
				}
			?> 
			<input id="del_num" type="text" value="" class="input num_alphaCk" placeholder="송장번호">
			<input id="deliver_date" type="text" value="" class="input numCk" placeholder="발송일자">
			<input type="button" value="일괄적용" class="base_btm reg" onClick="chgdelinfo();"><br/>
			<p style="color:red">* 송장번호는 에스크로 배송정보 연동을 위해 “-”없이 입력해주세요.</p>
		</td>
	</tr>
	<? } ?>
</table>
		...
		<td>
			<table width="100%" border="0" cellspacing="1" cellpadding="6" class="t_style">
				...
				<tr>
					...
					<td class="t_value" style="padding: 4px 0 4px 10px" colspan='5'>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" height="38">
	...
	<tr class="t_name">
		...
		<td align="right" style="padding:10px">
			<span class="price_color">상품합계 <span class="tbold"><?=number_format($order_info->prd_price)?></span> 원</span>
			<?=$discount_msg?>
			<?=$deliver_price_view?>
			<?=$coupon_msg?>
			<?=$reserve_msg?>
	...
	<tr class="t_name_ord">
		<td width="80%" class="Right">
			...
			<span class="tbold2"><?=number_format($order_info->total_price)?></span> <span class="price_size6">원</span>
		</td>
	</tr>
	<tr><td colspan="9" height="1" bgcolor="#e1e1e1"></td></tr>
	<tr><td height="10"></td></tr>
		...
		<td>
			<table width="100%" border="0" cellspacing="1" cellpadding="6" class="t_style">
				...
				<tr>
					...
					<? } else { ?>
						<select name="chg_status" style="width:150" class="select2">
							<option value="">--------------------</option>
						<?
						...
						?>
							...
							<option value="OC" <? if($order_info->status == "OC") echo "selected"; ?>>주문취소</option>
							<option value="">--------------------</option>
							<option value="RD" <? if($order_info->status == "RD") echo "selected"; ?>>환불요청</option>
							<option value="RC" <? if($order_info->status == "RC") echo "selected"; ?>>환불완료</option>
				...
				<?
				//2023-12-28 상품별 운송장번호 입력 외 배송정보 입력 노출
				}else{
				?>
				<tr>
					...
					<td class="t_value">
						...
						<br/>
						<p style="color:red">에스크로 배송정보 연동을 위해 “-”없이 입력해주세요</p>
					</td>
					<td class="t_name">발송일자</td>
					<td class="t_value">
						<input name="deliver_date" id="deliver_date" type="text" value="<?=$order_info->deliver_date?>" class="input">
				...
				<?
				} //2023-12-28 상품별 운송장번호 입력 외 배송정보 입력 노출 끝
				?>
				<!--
				<tr>
					<td class="t_name"></td>
			...
			<table width="100%" border="0" cellspacing="1" cellpadding="6" class="t_style">
				...
				<tr>
					...
					<? $post = $order_info->rece_post; ?>
						...
						<input type="button" value="우편번호검색" class="base_btn2"  onClick="searchZip('rece_');">
					</td>
				</tr>
				<tr>
					<td class="t_name">주소</td>
						...
						<table width="100%" border="0" cellspacing="0" cellpadding="0">
							...
							<tr>
								<td width="100%">
									...
									<table id="tax00" style="display:<? if($order_info->tax_type == "N") echo ""; else echo "none"; ?>" width="60%" border="0" cellspacing="1" cellpadding="6" class="t_style">
										<tr height="35">
											...
											<td width="50%" class="t_value_proof" colspan="3">
												...
												<label><input type="radio" name="tax_pub" value="Y" <? if($tax_info['tax_pub'] == "Y") echo "checked" ?>> 발급완료</label>
												<label><input type="radio" name="tax_pub" value="N" <? if(($tax_info['tax_pub'] == "N") || empty($tax_info['tax_pub'])) echo "checked" ?>> 발급대기</label>
											</td>
										</tr>
									</table>
									<br>
									...
									<table id="tax02" style="display:<? if($order_info->tax_type ==  "C") echo ""; else echo "none"; ?>" width="60%" border="0" cellspacing="1" cellpadding="6" class="t_style">
										<tr height="35">
											<td width="10%" class="t_name_proof">&nbsp; 발급사유</td>
											<td width="50%" class="t_value_proof">
												<label><input type="radio" name="cash_type" value="C" <? if($tax_info['cash_type'] == "C") echo "checked" ?>> 사업자 지출증빙용</label>
												<label><input type="radio" name="cash_type" value="P" <? if($tax_info['cash_type'] == "P") echo "checked" ?>> 개인소득 공제용</label>
											</td>
										</tr>
										<tr height="35">
											<td class="t_name_proof">&nbsp; 신청정보</td>
											<td class="t_value_proof">
												<label><input type="radio" name="cash_type2" value="CARDNUM" <? if($tax_info['cash_type2'] ==  "CARDNUM") echo "checked" ?> onclick="taxType('CARDNUM')"> 현금영수증 카드번호</label>
												<label><input type="radio" name="cash_type2" value="COMNUM" <? if($tax_info['cash_type2'] == "COMNUM") echo "checked" ?> onclick="taxType('COMNUM')"> 사업자 등록번호</label>
												<label><input type="radio" name="cash_type2" value="HPHONE" <? if($tax_info['cash_type2'] == "HPHONE") echo "checked" ?> onclick="taxType('HPHONE')"> 휴대전화번호</label>
												<label><input type="radio" name="cash_type2" value="RESNO" <? if($tax_info['cash_type2'] == "RESNO") echo "checked" ?> onclick="taxType('RESNO')"> 주민등록번호</label>
											</td>
										</tr>
...
<? if(($oper_info['pay_agent'] == "KCP") && ($order_info->paymethod ==  "PC")) { ?>
<br>
<table border="0" cellspacing="0" cellpadding="2">
	<tr>
