...
function basketDelete(num){
	...
	if(selorder == ""){
		alert("위시리스트에 담겨진 상품을 선택하지 않았습니다.");
		return;
	}else{
		if(confirm("선택한 위시리스트상품을 정말 삭제하시겠습니까?")){
			document.location = "prd_save.php?mode=wishlistDel&selorder=" + selorder + "&page="+num+"&<?=$param?>";
		}else{
			return;
		}
	}
...
<table border="0" cellspacing="0" cellpadding="2">
	<tr>
		...
		<td valign="bottom" class="tit">위시리스트 통계</td>
		<td width="2"></td>
		<td valign="bottom" class="tit_alt">위시리스트에 담긴 전체상품을 관리합니다.</td>
	</tr>
</table>
<br>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
	...
	$sql = "SELECT wb.*, wp.del_type, wp.del_price, wp.prdname, wp.prdimg_R 
			FROM (select * from wiz_wishlist group by prdcode) as wb
			inner join wiz_product as wp on wb.prdcode = wp.prdcode 
			left join (select * from wiz_cprelation group by prdcode) as wcp on wp.prdcode = wcp.prdcode
			left join wiz_category as wc on wcp.catcode = wc.catcode			
			where 1 $searchopt_sql order by idx desc limit $start, $rows";
	$result = query($sql) or error("sql error");

	while(($brow = sql_fetch_arr($result)) && $rows){

		if(!@file($_SERVER['DOCUMENT_ROOT']."/twcenter/data/prdimg/".$brow['prdimg_R'])) $brow['prdimg_R'] = "/twcenter/images/noimg_S.gif";
		else $brow['prdimg_R'] = "/twcenter/data/prdimg/".$brow['prdimg_R'];
		...
		if(!isset($brow['optcode3'])) $brow['optcode3'] = '';
		...
		if(!isset($brow['optcode4'])) $brow['optcode4'] = '';
		...
		if(!isset($brow['optcode8'])) $brow['optcode8'] = '';
		...
		if(!isset($brow['optcode9'])) $brow['optcode9'] = '';
		...
		if(!isset($brow['optcode10'])) $brow['optcode10'] = '';
		...
		if(!isset($brow['optcode11'])) $brow['optcode11'] = '';
	...
	<tr>
		...
		<td><a href="<?=$purl?>?ptype=view&prdcode=<?=$brow['prdcode']?>" target="_blank"><?=$optcode?></a></td>
