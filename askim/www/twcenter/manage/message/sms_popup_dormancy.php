...
if (count($selected_count) > 1) {
	if(isset($seluser)){
		...
		while($array_selected[$i]){
			...
			$_gdata = sql_fetch( "SELECT hphone FROM wiz_member_dormancy WHERE id='{$tmp_id}' " );
	...
	$_gdata = sql_fetch( "SELECT hphone FROM wiz_member_dormancy WHERE id='{$seluser}' " );
...
$sms_sub = "휴면처리공지 안내문자";
$sms_msg = stripslashes($row->sms_msg);
$sms_msg = info_replace($site_info, $re_info, $sms_msg);

?>
<!DOCTYPE html>
<html lang="ko">
<head>
<title>:: 휴면처리공지 SMS발송 ::</title>
		...
		<tr>
		  <td class="sms_bg">
			<table width="100%" border="0" cellspacing="0" cellpadding="0" align="center">
			  ...
			  ?>
			  <tr>
				<td align="center"><input type="text" class="sms_number" placeholder="발신제목" name="strSubject" value="<?php echo $sms_sub ?>"></td>
			  </tr>
			  <?php } ?>
			  <tr>
				<td align="center" class="remaining"><textarea name="message" id="message" class="sms_text" placeholder="메시지를 입력해주세요" ><?php echo $sms_msg ?></textarea>
