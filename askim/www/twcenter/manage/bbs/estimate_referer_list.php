...
if(!empty($list_cnt))	$param .= "&list_cnt=$list_cnt";
...
function popOpen(ip,key){
	if(key == ""){
		...
		var url = "/twcenter/manage/connect/connect_search_ip.php?query="+ip;
...
<ul class="tab_con">
	<li><a href="/twcenter/manage/bbs/list.php?ptype=list&code=<?=$code?>&menucode=BBS">견적문의</a></li>
	<li class="hover"><a href="/twcenter/manage/bbs/estimate_referer_list.php?menucode=BBS&code=estimate">견적문의 접수 분석</a></li>
</ul>
...
<table width="100%" cellspacing="1" cellpadding="3" border="0" class="t_style">
	<tr>
		...
		<td width="85%" class="t_value">
			...
			<select name="list_cnt" class="input" style="height:35px; float:right;" id="list_cnt" onchange="location.href='?<?=$param?>&list_cnt='+this.value">
					<option value="10" <?php if($list_cnt == 10 || $list_cnt == "") echo "selected"; ?>>10개씩보기</option>
					<option value="20" <?php if($list_cnt == 20) echo "selected"; ?>>20개씩보기</option>
					<option value="30" <?php if($list_cnt == 30) echo "selected"; ?>>30개씩보기</option>
					<option value="50" <?php if($list_cnt == 50) echo "selected"; ?>>50개씩보기</option>
					<option value="100" <?php if($list_cnt == 100 ) echo "selected"; ?>>100개씩보기</option>
			</select>
		</td>
	</tr>
...
<table width="100%" cellspacing="1" cellpadding="3" border="0">
	<tr>
		<td align="center">
			...
			<input type="button" value="전체목록" class="search_default" onclick="location.href='/twcenter/manage/bbs/estimate_referer_list.php?&menucode=BBS&code=estimate'">&nbsp;
...
<?php
	...
	$page_count = ceil($total/$list_cnt);
	$start = ($page-1)*$list_cnt;
	$no = $total-$start;
	$rows = $list_cnt;
