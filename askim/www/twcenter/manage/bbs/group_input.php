...
<?
	...
	if(!strcmp($mode, "grpupdate") && $idx) {
		$grp = sql_fetch("select * from wiz_bbs_grp where idx='".$idx."'");
		$grpname = $grp['grpname'];
		$prior = $grp['prior'];
	} else {
		$row_prior = sql_fetch("select max(prior) as prior from wiz_bbs_grp");
...
<form name="frm" action="bbs_save.php" method="post" onSubmit="return inputCheck(this);">
