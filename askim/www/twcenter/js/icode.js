 ...
 */ 
function add(str) {
	document.MsgForm.send_msg.focus();
	document.MsgForm.send_msg.value+=str; 
	ChkLen();
	return;
}
...
function ChkLen() {
	...
	var len = MsgForm.send_msg.value.length;
	for (i=0;i<len;i++){
		pos = MsgForm.send_msg.value.charAt(i);
	...
	if(isNewSubject && msglen <= 30) MsgForm.strSubject.value = MsgForm.send_msg.value;

	MsgForm.send_msgCount.value = msglen;
...
function addSelectBox(name, tel) {
	...
	index = MsgForm.recv_num.options.length;
	...
	if(phoneRe.exec(tel)){
		...
		for (i=0; i < index ; i++) {
			if (MsgForm.recv_num.options[i].text == tel) {
				alert(tel+"은 이미 리스트에 포함되어 있습니다.");
				return;
			}
		}

		MsgForm.recv_num.options.length = index+1;
		MsgForm.recv_num.options[index].value = tel +";";
		MsgForm.recv_num.options[index].text = tel;
		MsgForm.recv_num.options.length = index+1;
	} 
}   
...
function varcheck() {
	...
	// 일괄 문구 치환
	MsgForm.send_msg.value = MsgForm.send_msg.value.replace(/\u200b/g, '');

	var varsend_msg = document.MsgForm.send_msg.value;
//	var varStrSubject = document.MsgForm.strSubject.value;
	var varsend_num = document.MsgForm.send_num.value;

	// 받는 사람의 전화번호가 들어가 있는지 확인합니다.
	if (document.MsgForm.recv_num.length < 2){
	...
	if (varsend_num  == null || varsend_num == ""){
		alert("회신번호를 입력하세요");
		document.MsgForm.send_num.focus();
		return false;
	}
	...
	if (varsend_msg == ""){
		...
		document.MsgForm.send_msg.focus();
		return false;
	} else if (varsend_msg.length > 1500) {
	...
	document.MsgForm.strTelList.value = "";
	for (k=1; k < document.MsgForm.recv_num.length; k++) { 
		document.MsgForm.strTelList.value += document.MsgForm.recv_num.options[k].value;
	}
