...
// 상단파일
@include $_SERVER['DOCUMENT_ROOT'].'/$skin_dir/member_head.php';

// 게시물 쿼리
if($searchopt) $search_sql = " and $searchopt like '%$searchkey%' ";

$sql = "select * from wiz_member where id != '$wiz_session['id']' $search_sql order by idx desc";
...
$sql = "select * from wiz_member where id != '$wiz_session['id']' $search_sql order by idx desc limit $start, $rows";
$result = query($sql) or error("sql_error");
...
while($row = sql_fetch_arr($result)){
	...
	// 글목록파일
	@include $_SERVER['DOCUMENT_ROOT'].'/$skin_dir/member_body.php';
...
if($total <= 0){
	echo "<tr><td height='25' align='center' colspan='20'>등록된 회원이 없습니다.</td></tr>";
}

$grp_btn = "<img src='".$skin_dir."/image/btn_msg.gif' onClick='grpMessage()' style='cursor:hand'>";
$frd_btn = "<img src='".$skin_dir."/image/btn_fadd.gif' onClick='addFriend()' style='cursor:hand'>";

// 하단파일
@include $_SERVER['DOCUMENT_ROOT'].'/$skin_dir/member_foot.php';
