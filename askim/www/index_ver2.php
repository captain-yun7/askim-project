...
$code_page='main';
$_GET['bg'] = 1;
include_once $_SERVER["DOCUMENT_ROOT"]."/head_ver2.php";
...
<script type="text/javascript" src="/comm/js/ScrollTrigger.min.js"></script>
	...
	<script>
		...
		if (typeof lastScrollTop2 === 'undefined') {
		...
		window.addEventListener('scroll', () => {
		  ...
		  document.querySelectorAll('.usp-item').forEach(item => {
			const img = item.querySelector('.usp_obj_img');
			if (!img) return;

			if (item.classList.contains('active')) {
			  if (st > lastScrollTop2) {
				img.classList.add('on');
				img.classList.remove('slow'); // 스크롤 중엔 정상 속도
			  } else {
		  ...
		  scrollTimeout = setTimeout(() => {
			// on 클래스 제거 대신 slow로 전환하여 부드럽게 멈춤
			document.querySelectorAll('.usp-item .usp_obj_img.on').forEach(img => {
